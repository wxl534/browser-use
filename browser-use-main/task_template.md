# {{ site_label }} “{{ title_prefix }}” 图片下载任务
...
只允许访问以下官方域名及其页面内直接引用的官方图片资源（域名后缀白名单：{{ allowed_hosts_json }}）：
...
1. 目标是 **{{ target_count }} 张成功保存的有效图片**，不是 {{ target_count }} 次尝试。
2. 搜索关键词固定为 `{{ keyword }}`。
3. 按搜索结果顺序处理，不要随机挑选。
...
download_current_idp_search_page_images(
  ...
  allowed_host_suffixes={{ allowed_hosts_json }},
...
#### 逐 item 单项流程（{{#if is_batch}}批量失败项兜底；{{/if}}发号 + 通用下载）

0. 调用 `next_search_item({{#if is_generic}}keyword="{{ keyword }}"{{/if}})` 获取“本页下一个尚未处理的 item 的序号 + 详情页 URL”。该工具按搜索结果页 DOM 顺序（从上到下、从左到右）枚举本页 item，并交叉核对 `image_record.jsonl` 已下载记录，自动跳过已处理项，避免错位、跳过、重复循环。处理完一个 item 后，把它的详情页 URL 作为 `mark_done_url` 传回再发下一个。{{#if is_generic}}（若本站点未注册 item 选择器，必须显式传入 `item_selector="{{ item_selector }}"`。）{{/if}}
1. 打开 `next_search_item` 发回的那个未处理详情页。
2. 提取标题、详情页 URL、馆藏号、作者/制作者、年代、地点、分类、说明文字。
3. 确认页面与 `{{ keyword }}` 语境相关。只要标题、说明、地点、来源等字段存在相关证据即可视为相关。
...
{{#if is_batch}}
8. 当前页处理完后，如果总保存数量仍不足 {{ target_count }}，必须用 `navigate_idp_search_page(keyword="{{ keyword }}", page=下一页, limit=50)` 跳转，不要手写搜索 URL。
{{/if}}
