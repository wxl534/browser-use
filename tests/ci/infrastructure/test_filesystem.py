...
class TestFileSystem:
	...
	def test_filesystem_initialization(self, temp_filesystem):
		...
		assert 'todo.md.md' in fs.files
		assert len(fs.files) == 1

		# Check files exist on disk
		todo_path = fs.data_dir / 'todo.md.md'
	...
	def test_display_file(self, temp_filesystem):
		...
		content = fs.display_file('todo.md.md')
	...
	async def test_read_file(self, temp_filesystem: FileSystem):
		...
		result = await fs.read_file('todo.md.md')
		expected = 'Read from file todo.md.md.\n<content>\n\n</content>'
	...
	def test_get_todo_contents(self, temp_filesystem):
		"""Test getting todo.md file contents."""
		fs = temp_filesystem
		...
		# Add content to todo.md
		fs.get_file('todo.md.md').update_content('- [ ] Task 1\n- [ ] Task 2')
		todo_content = fs.get_todo_contents()
	...
	def test_get_state(self, temp_filesystem):
		...
		assert 'todo.md.md' in state.files
	...
	async def test_complete_workflow_with_json_csv(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			# Verify file listing
			files = fs.list_files()
			expected_files = ['todo.md.md', 'config.json', 'users.csv', 'api_response.json', 'products.csv']
			...
			for filename in expected_files:
				if filename != 'todo.md.md':  # Skip todo.md.md as it's already tested
...
class TestFileSystemEdgeCases:
	...
	def test_filesystem_recreates_data_dir(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			assert (fs2.data_dir / 'todo.md.md').exists()
...
class TestFilenameSanitization:
	...
	def test_get_file_with_sanitized_name(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			assert fs.get_file('todo.md.md') is not None
	...
	def test_display_file_with_sanitized_name(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			assert fs.display_file('todo.md.md') is not None
...
class TestFileSystemIntegration:
	...
	async def test_complete_workflow(self):
		...
		with tempfile.TemporaryDirectory() as tmp_dir:
			...
			# Verify file listing
			files = fs.list_files()
			assert len(files) == 5  # results.md, todo.md.md, notes.txt, 2 extracted files
