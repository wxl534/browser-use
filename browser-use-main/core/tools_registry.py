...
import aiohttp
import anyio
from pydantic import BaseModel, Field  # noqa: F401 (re-export: legacy/site_tools.py 依赖)
...
# 常量与工具参数模型已拆到 registry_views.py;此处 re-export 维持向后兼容
# (worker.py / tool_actions/* 仍 `from tools_registry import ...` 这些符号).
from registry_views import (  # noqa: E402,F401
    EXT_TO_PIL_FORMAT,
    GENERAL_IMAGE_METHODS,
    GENERAL_IMAGE_STRATEGY_LOCK_THRESHOLD,
...
def configure_runtime_paths(run_dir: Path, image_dir: Path | None = None, data_dir: Path | None = None) -> None:
    ...
    # 双写:同步到 runtime_paths(拆出去的子模块经 getter 读 live 值)
    runtime_paths.set_paths(RUN_DIR, IMAGE_DIR, AGENT_DATA_DIR)
    ...
    *,
    ...
    try:
        ...
        record = {
            ...
            'overview': {str(k): str(v) for k, v in (overview or {}).items() if str(k).strip() and str(v).strip()},
            'source_page': (int(source_page) if isinstance(source_page, int) or (isinstance(source_page, str) and source_page.isdigit()) else None),
